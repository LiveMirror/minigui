...
public:
	ELabel():EControl(NULL,EG_RedrawFunc_DefaultELabel),m_pLabelString(NULL){}
	ELabel(EPosition pos,char* pstring,unsigned int labelWidth);
	ELabel(EG_CallbackFunc cf,EPosition pos,char* pstring,unsigned int labelWidth);

	friend void EG_RedrawFunc_DefaultELabel(EControl* control);

	void SetLabelString(char* pstring){
		this->m_pLabelString=pstring;
	}
...
class ENumberLabel:public EControl{
public:
	ENumberLabel():EControl(NULL,EG_RedrawFunc_DefaultENumberLabel),m_uLabelNumber(0){}
	ENumberLabel(EPosition pos,int number,unsigned int labelWidth);
	ENumberLabel(EG_CallbackFunc cf,EPosition pos,int number,unsigned int labelWidth);

	friend void EG_RedrawFunc_DefaultENumberLabel(EControl* control);

	void SetLabelNumber(int number){
		this->m_uLabelNumber=number;;
	}
...
protected:
	static const int MaxStringLength=40;
