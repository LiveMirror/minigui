...
void EG_Graphic_DrawWidthLine(unsigned int x0,unsigned int y0,unsigned int x1,unsigned int y1,unsigned char lineWidth){
   ...
   /* set the increase direction along x axie*/
	if(dx>0){
		...
		if(dx<0){
			dx_sym =(char)-1;				// dx<0, set dx_sym=-1
   ...
   /* set the increase direction along x axie*/
	if(dy>0){
		...
		if(dy<0){
			dy_sym =(char)-1;				// dy<0, set dy_sym=-1
		}else{
...
void EG_Graphic_DrawNumber(unsigned int x,unsigned int y,int number){
	...
	if((q>0)&&(q<10)){
		first=0;
		q+=0x30;
		EG_Graphic_DrawChar(x,y,(char)q);
		x+=8;	
	}
	q=r/1000;
	r=r%1000;
	...
	r+=0x30;
	EG_Graphic_DrawChar(x,y,(char)r);
}
...
#ifdef DOT_MATRIX_LCD
#ifdef EG_NO_EBUFFER
		EG_LCDFlushPoint(color,pos.m_uX+n,pos.m_uY);
#else
		EG_LCD_Buffer[pos.m_uY][pos.m_uX+n]=color;
#endif  //EG_NO_EBUFFER
#else
...
#ifdef DOT_MATRIX_LCD
#ifdef EG_NO_EBUFFER
		EG_LCDFlushPoint(color,pos.m_uX,pos.m_uY+n);
#else
		EG_LCD_Buffer[pos.m_uY+n][pos.m_uX]=color;
#endif  //EG_NO_EBUFFER
#else
...
void EG_Graphic_DrawEmptyCircle(const EPosition& center,unsigned int radius,const EColor& color){
...
#ifdef DOT_MATRIX_LCD
#ifdef EG_NO_EBUFFER
		EG_LCDFlushPoint(color,draw_x0,draw_y0);
#else
		EG_LCD_Buffer[draw_y0][draw_x0]=color;// 90degree
#endif  //EG_NO_EBUFFER
#else
...
#ifdef DOT_MATRIX_LCD
#ifdef EG_NO_EBUFFER
		EG_LCDFlushPoint(color,draw_x2,draw_y2);
#else
		EG_LCD_Buffer[draw_y2][draw_x2]=color;// 270degree
#endif  //EG_NO_EBUFFER
#else
...
#ifdef DOT_MATRIX_LCD
#ifdef EG_NO_EBUFFER
		EG_LCDFlushPoint(color,draw_x4,draw_y4);
#else
		EG_LCD_Buffer[draw_y4][draw_x4]=color;// 0degree
#endif  //EG_NO_EBUFFER
#else
...
#ifdef DOT_MATRIX_LCD
#ifdef EG_NO_EBUFFER
		EG_LCDFlushPoint(color,draw_x5,draw_y5);
#else
		EG_LCD_Buffer[draw_y5][draw_x5]=color;// 180degree
#endif  //EG_NO_EBUFFER
#else
	...
	while(xx<yy){
		...
		if( (draw_x0<=EG_COLUMN)&&(draw_y0>=0) ){
			...
			EG_LCDFlushPoint(color,draw_x0,draw_y0);
#else
			EG_LCD_Buffer[draw_y0][draw_x0]=color;
#endif  //EG_NO_EBUFFER
#else
...
#ifdef DOT_MATRIX_LCD
#ifdef EG_NO_EBUFFER
			EG_LCDFlushPoint(color,draw_x1,draw_y1);
#else
			EG_LCD_Buffer[draw_y1][draw_x1]=color;
#endif  //EG_NO_EBUFFER
#else
		...
		if( (draw_x2<=EG_COLUMN)&&(draw_y2<=EG_ROW) ){
			...
			EG_LCDFlushPoint(color,draw_x2,draw_y2);
#else
			EG_LCD_Buffer[draw_y2][draw_x2]=color;
#endif  //EG_NO_EBUFFER
#else
...
#ifdef DOT_MATRIX_LCD
#ifdef EG_NO_EBUFFER
			EG_LCDFlushPoint(color,draw_x3,draw_y3);
#else
			EG_LCD_Buffer[draw_y3][draw_x3]=color;
#endif  //EG_NO_EBUFFER
#else
		...
		if( (draw_x4<=EG_COLUMN)&&(draw_y4>=0) ){
			...
			EG_LCDFlushPoint(color,draw_x4,draw_y4);
#else
			EG_LCD_Buffer[draw_y4][draw_x4]=color;
#endif  //EG_NO_EBUFFER
#else
		...
		if( (draw_x5>=0)&&(draw_y5>=0) ){
			...
			EG_LCDFlushPoint(color,draw_x5,draw_y5);
#else
			EG_LCD_Buffer[draw_y5][draw_x5]=color;
#endif  //EG_NO_EBUFFER
#else
...
#ifdef DOT_MATRIX_LCD
#ifdef EG_NO_EBUFFER
			EG_LCDFlushPoint(color,draw_x6,draw_y6);
#else
			EG_LCD_Buffer[draw_y6][draw_x6]=color;
#endif  //EG_NO_EBUFFER
#else
...
#ifdef DOT_MATRIX_LCD
#ifdef EG_NO_EBUFFER
			EG_LCDFlushPoint(color,draw_x7,draw_y7);
#else
			EG_LCD_Buffer[draw_y7][draw_x7]=color;
#endif  //EG_NO_EBUFFER
#else
...
#ifdef DOT_MATRIX_LCD
#ifdef EG_NO_EBUFFER
				EG_LCDFlushPoint(color,pos.m_uX+7-temp2,pos.m_uY+temp1);
#else
				EG_LCD_Buffer[pos.m_uY+temp1][pos.m_uX+7-temp2]=color;
#endif  //EG_NO_EBUFFER
#else
...
void EG_Graphic_DrawNumber(const EPosition& pos,int number,const EColor& color){
	...
	pos1=pos;
	if(number<0){
		EG_Graphic_DrawChar(pos1,'-',color);
		pos1.m_uX=pos1.m_uX+8;	
	}
	q=number/10000;
	r=number%10000;
	if((q>0)&&(q<10)){
		first=false;
		q+=0x30;
		EG_Graphic_DrawChar(pos1,(char)q,color);
		pos1.m_uX=pos1.m_uX+8;	
	}
	q=r/1000;
	r=r%1000;
	...
	r+=0x30;
	EG_Graphic_DrawChar(pos1,(char)r,color);
}
...
void EG_Graphic_DrawBitmap(const EPosition& pos,EBitmap* pBitmap){
	...
	for(unsigned int m=0;m<pBitmap->m_uHeight;m++){
		for(unsigned int n=0;n<pBitmap->m_uWidth;n++){
			...
			temp.m_cHighByte=*tempp++;
			temp.m_cLowByte=*tempp++;
#else
			temp.m_cRed=*tempp++;
			temp.m_cGreen=*tempp++;
			temp.m_cBlue=*tempp++;
...
#ifdef EG_NO_EBUFFER
				if(font.m_cFontSize==1){
					EG_LCDFlushPoint(color,pos.m_uX+EG_FontWidth[font.m_cFontNumber]-1-temp2,pos.m_uY+temp1);
				}else if (font.m_cFontSize==2){
					EG_LCDFlushPoint(color,pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)<<1)-1,pos.m_uY+(temp1<<1));
					EG_LCDFlushPoint(color,pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)<<1),pos.m_uY+(temp1<<1)+1);
					EG_LCDFlushPoint(color,pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)<<1)-1,pos.m_uY+(temp1<<1)+1);
					EG_LCDFlushPoint(color,pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)<<1),pos.m_uY+(temp1<<1));			
				}else{
					EG_LCDFlushPoint(color,pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)*3)-1,pos.m_uY+temp1*3);
					EG_LCDFlushPoint(color,pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)*3),pos.m_uY+temp1*3);
					EG_LCDFlushPoint(color,pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)*3)+1,pos.m_uY+temp1*3);		
					EG_LCDFlushPoint(color,pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)*3)-1,pos.m_uY+temp1*3-1);
					EG_LCDFlushPoint(color,pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)*3),pos.m_uY+temp1*3-1);
					EG_LCDFlushPoint(color,pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)*3)+1,pos.m_uY+temp1*3-1);		
					EG_LCDFlushPoint(color,pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)*3)-1,pos.m_uY+temp1*3+1);
					EG_LCDFlushPoint(color,pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)*3),pos.m_uY+temp1*3+1);
					EG_LCDFlushPoint(color,pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)*3)+1,pos.m_uY+temp1*3+1);		
...
#else
				if(font.m_cFontSize==1){
					EG_LCD_Buffer[pos.m_uY+temp1][pos.m_uX+EG_FontWidth[font.m_cFontNumber]-1-temp2]=color;			
				}else if (font.m_cFontSize==2){
					EG_LCD_Buffer[pos.m_uY+(temp1<<1)][pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)<<1)-1]=color;			
					EG_LCD_Buffer[pos.m_uY+(temp1<<1)+1][pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)<<1)]=color;			
					EG_LCD_Buffer[pos.m_uY+(temp1<<1)+1][pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)<<1)-1]=color;			
					EG_LCD_Buffer[pos.m_uY+(temp1<<1)][pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)<<1)]=color;			
				}else{
					EG_LCD_Buffer[pos.m_uY+temp1*3][pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)*3)-1]=color;			
					EG_LCD_Buffer[pos.m_uY+temp1*3][pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)*3)]=color;			
					EG_LCD_Buffer[pos.m_uY+temp1*3][pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)*3)+1]=color;			
					EG_LCD_Buffer[pos.m_uY+temp1*3+1][pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)*3)-1]=color;			
					EG_LCD_Buffer[pos.m_uY+temp1*3+1][pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)*3)]=color;			
					EG_LCD_Buffer[pos.m_uY+temp1*3+1][pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)*3)+1]=color;			
					EG_LCD_Buffer[pos.m_uY+temp1*3+2][pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)*3)-1]=color;			
					EG_LCD_Buffer[pos.m_uY+temp1*3+2][pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)*3)]=color;			
					EG_LCD_Buffer[pos.m_uY+temp1*3+2][pos.m_uX+((EG_FontWidth[font.m_cFontNumber]-temp2)*3)+1]=color;			
				}	
#endif  //EG_NO_EBUFFER
#endif  //DOT_MATRIX_LCD			
...
void EG_Graphic_DrawFontNumber(const EPosition& pos,int number,const EColor& color,const EFont& font){
	...
	pos1=pos;
	if(number<0){
		EG_Graphic_DrawFontChar(pos1,'-',color,font);
		pos1.m_uX=pos1.m_uX+8;	
	}
	q=number/10000;
	r=number%10000;
	if((q>0)&&(q<10)){
		first=false;
		q+=0x30;
		EG_Graphic_DrawFontChar(pos1,(char)q,color,font);
		pos1.m_uX=pos1.m_uX+8;	
	}
	q=r/1000;
	r=r%1000;
	...
	r+=0x30;
	EG_Graphic_DrawFontChar(pos1,(char)r,color,font);
}
