...
#ifdef EXTERNAL_LCD_DRIVER

/*
    How to write custom LCD driver:
        void EG_LCDInit()  ->  Initialize LCD,   
		    1 Config MCU GPIO direction
			2 reset LCD module
			3 send cmd to init it
		void EG_LCDClearScreen()  ->  Erase the LCD RAM,so the screen is clear
			1 set LCD inner buffer to zero 
		void EG_LCDFlushBuffer()  ->  copy the EG_LCD_Buffer into LCD RAM 	
			1 set LCD inner buffer to EG_LCD_Buffer
*/
